...
{
    switch (off) {
    case PIC32MK_DCHxCON:  return &ch->con;
    case PIC32MK_DCHxECON: return &ch->econ;
    case PIC32MK_DCHxINT:  return &ch->ireg;
    case PIC32MK_DCHxSSA:  return &ch->ssa;
    case PIC32MK_DCHxDSA:  return &ch->dsa;
    case PIC32MK_DCHxSSIZ: return &ch->ssiz;
    case PIC32MK_DCHxDSIZ: return &ch->dsiz;
    case PIC32MK_DCHxSPTR: return &ch->sptr;
    case PIC32MK_DCHxDPTR: return &ch->dptr;
    case PIC32MK_DCHxCSIZ: return &ch->csiz;
    case PIC32MK_DCHxCPTR: return &ch->cptr;
    case PIC32MK_DCHxDAT:  return &ch->dat;
    default:               return NULL;
    }
}
...
static uint64_t dma_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    /* Global registers */
    if (base == PIC32MK_DMACON_OFFSET) return s->con;
    if (base == PIC32MK_DMASTAT_OFFSET) return s->stat;
    if (base == PIC32MK_DMAADDR_OFFSET) return s->addr;
