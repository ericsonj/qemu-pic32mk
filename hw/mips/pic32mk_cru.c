...
static uint32_t apply_sci(uint32_t old, uint32_t val, unsigned sub)
{
    switch (sub) {
    case 0x0: return val;
    case 0x4: return old | val;
    case 0x8: return old & ~val;
    case 0xC: return old ^ val;
    default:  return old;   /* unreachable */
    }
}
...
static uint64_t pic32mk_cru_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    switch (base) {
    case PIC32MK_CRU_OSCCON:  return s->osccon;
    case PIC32MK_CRU_OSCTUN:  return s->osctun;
    case PIC32MK_CRU_SPLLCON: return s->spllcon;
    case PIC32MK_CRU_UPLLCON: return s->upllcon;

    case PIC32MK_CRU_RCON:    return s->rcon;
    case PIC32MK_CRU_RSWRST:  return s->rswrst;
    case PIC32MK_CRU_RNMICON: return s->rnmicon;
    case PIC32MK_CRU_PWRCON:  return s->pwrcon;

    /* Reference clock outputs 1–4: CON then TRIM, 0x20 stride */
    case PIC32MK_CRU_REFO1CON:  return s->refo_con[0];
    case PIC32MK_CRU_REFO1TRIM: return s->refo_trim[0];
    case PIC32MK_CRU_REFO2CON:  return s->refo_con[1];
    case PIC32MK_CRU_REFO2TRIM: return s->refo_trim[1];
    case PIC32MK_CRU_REFO3CON:  return s->refo_con[2];
    case PIC32MK_CRU_REFO3TRIM: return s->refo_trim[2];
    case PIC32MK_CRU_REFO4CON:  return s->refo_con[3];
    case PIC32MK_CRU_REFO4TRIM: return s->refo_trim[3];

    /* Peripheral bus dividers — 0x10 stride starting at 0x100 */
    case PIC32MK_CRU_PB1DIV:  return s->pbdiv[0] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB2DIV:  return s->pbdiv[1] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB3DIV:  return s->pbdiv[2] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB4DIV:  return s->pbdiv[3] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB5DIV:  return s->pbdiv[4] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB6DIV:  return s->pbdiv[5] | PIC32MK_PBDIV_PBDIVRDY;
    case PIC32MK_CRU_PB7DIV:  return s->pbdiv[6] | PIC32MK_PBDIV_PBDIVRDY;
