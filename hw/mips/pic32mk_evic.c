...
{
    if (base == PIC32MK_EVIC_INTCON)  return &s->intcon;
    if (base == PIC32MK_EVIC_PRISS)   return &s->priss;
    if (base == PIC32MK_EVIC_INTSTAT) return &s->intstat;
    if (base == PIC32MK_EVIC_IPTMR)   return &s->iptmr;
