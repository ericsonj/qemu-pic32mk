...
/* Map to atomic EVIC_INT_SourceDisable/Restore (provided by plib_evic.c).
 * EVIC_INT_SourceDisable disables global interrupts, saves previous enable
 * state, disables the source, and restores global interrupts — returning the
 * saved state as bool.  This matches what the USB driver expects. */
#define SYS_INT_SourceDisable(source)         EVIC_INT_SourceDisable(source)
#define SYS_INT_SourceRestore(source, status) EVIC_INT_SourceRestore(source, status)
